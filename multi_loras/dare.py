...
def drop_and_rescale_model(
    ...
    with torch.no_grad():
        dare_params_dict = {}
        for param_name, param_value in tqdm(model_param_dict.items(), ncols=100):
        ...
        dare_model_weights = new_delta_weights.combine_with_pretrained_model(
            pretrained_model=pretrained_model, scaling_coefficient=scaling_coefficient
        )
...
def do_dare(args):
    ...
    base_model = AutoModelForCausalLM.from_pretrained(
        args.bae_model_name_or_path, device_map=args.device_map, trust_remote_code=True
    )
    tuned_model = AutoModelForCausalLM.from_pretrained(args.tuned_model_name_or_path, device_map=args.device_map, trust_remote_code=True)
    ...
    print(f"Do drop and rescale with {dare_kwargs=} with {args.tuned_model_name_or_path} ...")
